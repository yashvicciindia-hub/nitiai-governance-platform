...
root.addEventListener("click", (event: Event) => { const target = event.target as HTMLElement; const actionEl = target.closest("[data-action]") as HTMLElement; const scrollEl = target.closest("[data-scroll]") as HTMLElement; const navEl = target.closest("[data-nav]") as HTMLElement; if (scrollEl) navigateTo(scrollEl.dataset.scroll || "home"); if (navEl) { event.preventDefault(); navigateTo(navEl.dataset.nav || "home"); }
  ...
  const ecosystemNode = target.closest("[data-home-ecosystem]") as HTMLElement; if (ecosystemNode) { const index = Number(ecosystemNode.dataset.homeEcosystem); const selected = ecosystem[index]; document.querySelectorAll(".home-governance-tag").forEach((node) => node.classList.toggle("active", node === ecosystemNode)); const detail = document.getElementById("homeEcosystemDetail"); if (detail) { const [title, description, metadata] = selected; detail.innerHTML = `<div class="home-ecosystem-detail-inner"><span>SELECTED SYSTEM / ${title}</span><h3>${title}</h3><p>${description}</p><div class="home-ecosystem-meta"><div><span>ROLE</span><strong>${metadata.role}</strong></div><div><span>CONNECTED TO</span><strong>${metadata.connected}</strong></div></div><div class="home-ecosystem-relationship"><span>AI CONNECTION</span><p>${metadata.how}</p></div></div>`; } const flow = document.getElementById("homeGovernanceFlow"); if (flow) { const steps = selected[2].flow; flow.innerHTML = `<div class="home-governance-flow-step active">${steps[0]}</div><div class="home-governance-flow-arrow">↓</div><div class="home-governance-flow-step">${steps[1]}</div><div class="home-governance-flow-arrow">↓</div><div class="home-governance-flow-step">${steps[2]}</div><div class="home-governance-flow-arrow">↓</div><div class="home-governance-flow-step">${steps[3]}</div>`; } }
